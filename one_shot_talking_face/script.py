...
def test_with_input_audio_and_image(
    ...
    tp = torch.from_numpy(tp).unsqueeze(0).unsqueeze(0)
    ...
    with torch.no_grad():
        for frame_idx in range(bs):
            ...
            t["audio"] = audio_f[:, frame_idx]
            t["pose"] = poses[:, frame_idx]
            t["ph"] = ph_frames[:, frame_idx]
            t["id_img"] = img
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--audio_path", type=str, default=None, help="path of the input audio ( .wav )"
    )
    argparser.add_argument(
        "--phoneme_path",
        type=str,
