...
def myres2Dblock(
    ...
    return Res2dBlock(
        ...
        k_size,
        padding,
        activation_norm_type=normalize,
...
def draw_annotation_box(
    image, rotation_vector, translation_vector, color=(255, 255, 255), line_width=2
):
    """Draw a 3D box as annotation of pose"""

    camera_matrix = np.array(
        [[233.333, 0, 128], [0, 233.333, 128], [0, 0, 1]], dtype="double"
    )
...
class up_sample(nn.Module):
    ...
    def forward(self, x):
        x = self.interp(
            x, scale_factor=self.scale_factor, mode="linear", align_corners=True
        )
        return x
