...
class Audio2kpTransformer(nn.Module):
    def __init__(self, opt):
        ...
        self.audio_embedding = nn.Sequential(
            nn.ConvTranspose2d(1, 8, (29, 14), stride=(1, 1), padding=(0, 11)),
            BatchNorm2d(8),
            nn.ReLU(inplace=True),
            nn.Conv2d(8, 35, (13, 13), stride=(1, 1), padding=(6, 6)),
        )
        self.decodefeature_extract = nn.Sequential(
            mydownres2Dblock(self.decode_dim, 32),
            mydownres2Dblock(32, 64),
            mydownres2Dblock(64, 128),
            mydownres2Dblock(128, 256),
            mydownres2Dblock(256, 512),
        ...
        self.jacobian.bias.data.copy_(
            torch.tensor([1, 0, 0, 1] * self.opt.num_kp, dtype=torch.float)
        )
    ...
    def create_sparse_motions(self, source_image, kp_source):
        ...
        return sparse_motions.permute(0, 1, 4, 2, 3).reshape(
            bs, (self.opt.num_kp + 1) * 2, 64, 64
        )
    ...
    def forward(self, x, initial_kp=None):
        ...
        input_feature = (
            self.feature_extract(input_feature).unsqueeze(-1).reshape(bs, seqlen, 512)
        )
        ...
        decoder_feature = (
            self.decodefeature_extract(
                torch.cat(
                    (
                        decoder_feature,
                        initial_kp["feature_map"]
                        .unsqueeze(1)
                        .repeat(1, seqlen, 1, 1, 1)
                        .reshape(bs * seqlen, 35, 64, 64),
                    ),
                    dim=1,
                )
            )
            .unsqueeze(-1)
            .reshape(bs, seqlen, 512)
        )
        ...
        output_feature = self.transformer(
            self.opt, input_feature, decoder_feature, posi_em
        )[-1, self.opt.num_w]

        out["value"] = self.kp(output_feature).reshape(bs, self.opt.num_kp, 2)
        out["jacobian"] = self.jacobian(output_feature).reshape(
            bs, self.opt.num_kp, 2, 2
        )
