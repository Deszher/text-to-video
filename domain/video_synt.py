...
def create_phoneme(audio_path: str, text: str) -> str:
    ...
    response= subprocess.run(
        "pocketsphinx -phone_align yes single /app/"+audio_path+" | jq '"+PHONEME_JK+ "' > /app/"+output_path,
        capture_output=True,
        shell=True,
        cwd="/app",
        executable="/bin/bash",
    ...
    print("create phoneme code, output_path: ", output_path, ", stdout:", response.stdout, "stderr:", response.stderr)

    return output_path

def make_video(img_path: str, audio_path, phoneme_path: str) -> str:
    response= subprocess.run(
        "python test_script.py --img_path /app/" + img_path + " --audio_path /app/" + audio_path + " --phoneme_path /app/" + phoneme_path + " --save_dir /app/data",
        capture_output=True,
        shell=True,
    ...
    output_path = "data/" + os.path.basename(img_path)[:-4] + "_" + os.path.basename(audio_path)[:-4] + ".mp4"

    print("make video code, output_path: ", output_path, ", stdout:", response.stdout, "stderr:", response.stderr)
